...
from backend.models import Admin, Config, Book, Template, MandatorySubscription
...
class Content(Dropdown):
    class MandatorySubscriptionResource(Model):
        label = 'Подписки'
        fields = [
            'id',
            'channel',
        ]
        model = MandatorySubscription

    class BookResource(Model):
        label = 'Книги'
        model = Book
        filters = [
            filters.Input(name='title', label='Book search')
        ]
        fields = [
            'id',
            'book_id',
            'title',
            'public_year',
            Field(name='img_link', label='Image',
                  display=displays.Image(width='40')),
            Field(name='description', label='Description',
                  input_=inputs.Editor()),
            'tags',
        ]

    class TemplateResource(Model):
        label = 'Шаблоны'
        model = Template
        fields = [
            'id',
    ...
    label = "Content"
    icon = "fas fa-bars"
    resources = [BookResource, TemplateResource, MandatorySubscriptionResource]
